...
class TargetItem extends Container {
    getName: () => string;
...
class TargetsList extends Container {
    constructor(events: Events, args = {}) {
        ...
        const items = new Map<Target, TargetItem>();

        events.on('scene.elementAdded', (element: Element) => {
            if (element.type === ElementType.gaze_target) {
                const target = element as Target;
        ...
        events.on('selection.changed', (selection: Target) => {
            items.forEach((value, key) => {
                value.selected = key === selection;
            });
        });

        events.on('splat.name', (target: Target) => {
            const item = items.get(target);
            if (item) {
        ...
        this.on('removeClicked', (item: TargetItem) => {
            let target;
            for (const [key, value] of items) {
                if (item === value) {
                    target = key;
