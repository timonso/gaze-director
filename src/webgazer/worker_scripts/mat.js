...
export const mat = {
     ...
     * @param {Number} j0 - Initial column index
     * @param {Number} j1 - Final column index
     * @returns {Array} The sub-matrix matrix(i0:i1,j0:j1)
     */
     ...
     * @param matrix1
     * @param matrix2
     * @returns {Array.<Array.<Number>>} Matrix product, A * B
     */
    mult(matrix1, matrix2) {
        ...
        const X = new Array(matrix1.length),
     ...
     * @returns {Array.<Array.<Number>>} s*A
     */
    multScalar(A, s) {
        const rows = A.length;
        const cols = A[0].length;
     ...
     * @returns {Array.<Array.<Number>>} An m * n matrix with ones on the diagonal and zeros elsewhere.
     */
    identity(m, n = m) {
        const X = new Array(m);
     ...
     * @returns {Array.<Array.<Number>>} solution if A is square, least squares solution otherwiseis
     */
    solve(A, B) {
     ...
     * @param {Array.<Array.<Number>>} A - left matrix of equation to be solved
     * @param {Array.<Array.<Number>>} B - right matrix of equation to be solved
     * @returns {Array.<Array.<Number>>} X so that L*U*X = B(piv,:)
     */
    LUDecomposition(A, B) {
        const LU = new Array(A.length);
        ...
        const m = A.length;
        const n = A[0].length;
        const piv = new Array(m);
        for (var i = 0; i < m; i++) {
            piv[i] = i;
        }
        let pivsign = 1;
        let LUrowi = new Array();
     ...
     * @returns {Array.<Array.<Number>>} X - that minimizes the two norms of QR*X-B.
     */
    QRDecomposition(A, B) {
        // Initialize.
        const QR = new Array(A.length);
